...
10. [ADR-004：Windows Broker 安装、生产只读与 Unity Worker 进程边界（historical——已被 ADR-005 supersede，仅作历史 provenance）](ADR-004_WINDOWS_BROKER_INSTALLATION_AND_PRODUCTION_READ.md)
11. [ADR-005：普通用户 Broker/Worker 架构（Accepted，USER_MODE 路线规范）](ADR-005_USER_MODE_BROKER_WORKER_ARCHITECTURE.md)
12. [ADR-006：AI Provider 双通道路由（Accepted）](ADR-006_AI_PROVIDER_TWO_CHANNEL_ROUTING.md)
13. [ADR-007：受控项目写入（PROPOSED——独立审计进行中）](ADR-007_CONTROLLED_PROJECT_MUTATION.md)
14. [机器门禁实现](50_MACHINE_ENFORCEMENT.md)
15. [全项目开发经验库](60_ENGINEERING_LESSONS.md)
16. [视觉迭代、证据与经验递归](70_ITERATION_EVIDENCE_AND_LEARNING.md)
