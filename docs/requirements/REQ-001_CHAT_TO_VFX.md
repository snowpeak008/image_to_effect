...
2. **Unity 侧特效编译能力**（S1–S11 历史阶段产物）：以 Recipe JSON 为输入的校验器（`RecipeValidator`）与编译器（`VfxCompiler` 及各模板族编译器），能把合法 Recipe 幂等地编译为 Prefab，写入限定在 `Assets/VFX/Generated`。历史执行计划（`docs/EXECUTION_PLAN.md` S9）已验证过"文字需求 → AI 写 Recipe → Validate → 修复 → Build"的人工流程可行。
...
3. 草稿经用户显式确认后才提交构建；构建经 Unity 侧权威校验与受限写入路径产出 Prefab，写入范围限定 `Assets/VFX/Generated`（`Assets/VFX/Shared/**` 的处理见 9.1 风险 R-2）。
4. 全流程 fail-closed：任何一步失败都以稳定错误码呈现并停止，不静默降级、不换 provider、不绕过校验。
5. 全流程可审计但不泄密：日志只含稳定错误码与脱敏摘要，不含 prompt 原文、endpoint、secret。

## 4. 非目标

1. **不做任何"Assets 之外"或"Generated 之外"的自动写入**：AI 产物不写模板目录、不写 `ProjectSettings`、不写任意用户路径；模板族共享资产（`Assets/VFX/Shared/**`）的补建是否允许由 ADR-007 单独裁决，本需求默认禁止构建任务触碰它。
...
flowchart TD
  ...
  I --> J["Unity 侧重新校验 → Build → 写入 Assets/VFX/Generated"]
...
- **REQ-001-18** 所有构建写入必须落在 `VfxCompiler.GeneratedRoot`（`Assets/VFX/Generated`）之下；越界目标必须被拒绝（`E600` 行为）且有负向测试。模板目录、`Assets/VFX/Shared/**`、`ProjectSettings/**` 对构建任务只读。
...
| G-9 | 写入安全设计：路径 containment 细则、`Assets/VFX/Shared/**` 共享资产的处理、原子写入/回滚、fail-closed 行为 | R4（ADR-007，F2 的前置） | REQ-001-18 |

## 9. 风险与开放问题

### 9.1 调研发现的风险

- **R-1 写入根表述不一致（已解决，2026-08-29）**：主计划与 CODING_STANDARDS 曾写"`Assets/Generated`"，与代码实际的 `VfxCompiler.GeneratedRoot = "Assets/VFX/Generated"` 不一致；两份文档均已更正为 `Assets/VFX/Generated`，本文与代码、治理文档三方一致。`Assets/VFX/Shared/**` 的写入政策仍待 ADR-007 裁决（见 R-2）。
- **R-2 共享资产写入超出 Generated**：`Impact2DSharedLibrary`/`Area2DSharedLibrary` 的 `Ensure()` 会在 `Assets/VFX/Shared/<Family>` 下补建材质/网格并配置纹理导入。若构建任务触发这些路径，"写入限定 Generated"即被突破。本需求暂定构建任务对 Shared 只读（缺失即失败），最终由 ADR-007 裁决。
...
- When：执行构建
- Then：构建被 `E600` 行为拒绝并标记 Blocked；Generated 根外无任何新文件；上次成功产物不变
