...
> **勘误（主 agent 裁决，2026-08-29，F4 审计发现）**：本表 6 命令闭集与 REQ-002-17 的"批次级取消入口"存在内部不一致。裁决：F5 增补第 7 条命令 `vfxc batch cancel <batchId>`（在共享执行层实现，CLI/MCP 共用；§7 MCP 工具面相应增补 `vfx_cancel_batch`），REQ-002-04 的"全部 6 个命令"自此读作"全部 7 个"。F4 交付的 6 命令面按原文验收，无需返工。
