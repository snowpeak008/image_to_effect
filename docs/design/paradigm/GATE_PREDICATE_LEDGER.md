...
| 状态 | 数 |
|---|---|
| 已实现（EditMode 测试在库） | 43 |
| T3 待做（编译器 v2 接线 / 族落地时实现） | 109 |
| 条件豁免（显式，含到期卡号） | 4 |
| **合计** | **156** |

状态口径：`已实现`＝存在可运行的 EditMode 测试对该谓词的语义作构造性断言（可以是规格谓词的等价物，等价性在"测试名/备注"列说明）；`T3 待做`＝断言面（编译器 v2 产物 / sidecar 清单 / .vfx 模板）尚不存在；`条件豁免`＝断言面存在但按拍板留待后续卡。

## 1. 材质族（31）

| 编号 | 测试名 | 状态 |
|---|---|---|
| SG-1 | — sidecar 清单机制未落（HLSL 路线下"子图"= include 库，见 T2B_IMPL_REPORT §3-1；等价物为 AllowList `subgraphs` 行 sha256 校验 `AssetAllowListGateTests.WL1_*`） | 条件豁免（HLSL 路线 sidecar 等价物已由 WL-1 承载；.shadersubgraph sidecar 属 T3 若引入 Shader Graph 路线） |
| SG-2 | — | T3 待做（变体清单落地时） |
| SG-3 | — | T3 待做（变体清单落地时） |
| SG-4 | — | T3 待做（变体清单落地时） |
...
| SG-6 | — | T3 待做（图结构断言在 HLSL 路线无对应；linear 色彩空间由 PR-1 承载） |
...
| SG-8 | — | T3 待做（组合清单落地时） |
| MG-1 | — 主图 4 张未立（HLSL 路线逐变体 shader 承载，见 T2B_IMPL_REPORT §2.1） | 条件豁免（主图/变体分层属 T3 `SG_VfxLit` 引入时；当前逐变体 shader 的 URP 标签一致性由 ShaderLibrary_AllShadersImport 承载） |
| MG-2 | `TechniqueFamilyBoundaryGateTests.GlowShader_CarriesTheFullParameterSurface`（统一属性面探针，Glow 主图） | 已实现（部分面；其余主图属 T3） |
| MG-3 | — | T3 待做 |
| MG-4 | — | T3 待做 |
| MG-5 | — | T3 待做 |
| MV-1 | `ParadigmSampleGateTests.Sample_GlowStackPresent_AndBeatCoupled`（时间驱动输入非静态） | 已实现（样片面；编译器谓词属 T3） |
...
| MV-6 | `AssetAllowListGateTests.WL1_EveryListedEntryResolvesToAnExistingAssetWithMatchingGuidAndSha256`（shader GUID 白名单命中） | 已实现（清单面；产物面 WL-4 属 T3） |
| MV-7 | — | T3 待做（sampleCost 求和需变体清单） |
| MV-8 | `TechniqueFamilyBoundaryGateTests.RefractionShader_DeclaresBothRoutes` | 已实现 |
| MV-9 | — | T3 待做（2D 排序谓词需编译器产物） |
| GL-1 | `ParadigmSampleGateTests.Sample_GlowStackPresent_AndBeatCoupled`（Glow 子节点存在且计数正确） | 已实现（样片面） |
| GL-2 | `ParadigmSampleGateTests.Sample_GlowStackPresent_AndBeatCoupled`（Glow 材质 = GlowStack additive） | 已实现（样片面） |
| GL-3 | — | T3 待做（半径几何级数需编译报告） |
| GL-4 | — | T3 待做 |
| GL-5 | `ParadigmSampleGateTests.Sample_ComponentsAreInsideTheRecognizedClosedSet`（Glow 子节点组件闭集内） | 已实现（样片面） |
| GL-6 | — | T3 待做 |
| GL-7 | `ParadigmSampleGateTests.Sample_FitsThePmTierBudget`（layerCount 计入 C_over 档位判定） | 已实现（样片面） |
| PR-1 | `TechniqueFamilyBoundaryGateTests`（EditMode 全套跑在 Linear 色彩空间工程内；显式断言属 T3 编译器接线） | 条件豁免（工程设置已是 Linear；显式 PlayerSettings 断言随编译器 v2 落地，卡号 T3） |
...
| 编号 | 测试名 | 状态 |
|---|---|---|
| GP-1 | — | T3 待做（.vfx 模板未产出，T2B_IMPL_REPORT §6-1；fail-closed：AllowList `vfxTemplates` 为空数组，任何 .vfx 引用即 WL-1 拒绝） |
| GP-2 ~ GP-9 | — | T3 待做（同上） |
...
| CP-8 | — | T3 待做（产物面：SubEmitter 峰值合并计入 C_cpu 需编译器） |
| CP-9 | — | T3 待做（渲染器排序面需编译器产物） |
| CP-10 | — | T3 待做（Lights/ExternalForces 模块在产物中禁用需编译器产物；构建期强制已在 Configure） |

## 4. 伪粒子 UGUI（5）

| 编号 | 测试名 | 状态 |
|---|---|---|
| UI-1 ~ UI-5 | — | T3 待做（F 类 `mat_ui_*` 与 `SG_VfxCanvas` 属 P2 批次，T2B_IMPL_REPORT §6-2） |
...
| MS-9 | — | T3 待做（产物 sidecar `.mesh.json` 需编译器写入面） |
| MS-10 | — | T3 待做（同上） |
| MS-11 | — | T3 待做（Trail/Line 运行时组件产物面） |
| MS-12 | — | T3 待做（Cloth 产物面） |

## 6. 物理（9）

| 编号 | 测试名 | 状态 |
|---|---|---|
| PH-1 | `ParadigmSampleGateTests.Sample_ComponentsAreInsideTheRecognizedClosedSet`（刚体/碰撞体在闭集内且仅预破碎层） | 已实现（样片面） |
| PH-2 | `VfxControllerV2Tests.ResetForPool_RestoresDebrisToKinematicInitialPose`（初始 TRS 烘入数组 + kinematic 复位） | 已实现 |
| PH-3 | `ParadigmSampleGateTests.Shield_HitIntegrityAndBreakInterfaces_Work`（break 后碎块受控释放） | 已实现（样片面） |
| PH-4 ~ PH-9 | — | T3 待做（碰撞矩阵/excludeLayers/Cloth 系数面需编译器产物） |

## 7. 局部光族（9）

| 编号 | 测试名 | 状态 |
|---|---|---|
| LT-1 | `ParadigmSampleGateTests.Sample_LocalLightIsPresent_AndDriven`（L-1 等价：光组件 + 节拍器同节点） | 已实现（样片面） |
| LT-2 | `VfxLightBeatTests.Breathe_OscillatesWithinDepthBand` / `Strobe_IsHardOnOff`（L-2 等价：非 steady 模式曲线非常量） | 已实现 |
| LT-3 | — | T3 待做（烘档材质发光绑定需编译器） |
| LT-4 | `ParadigmSampleGateTests.Sample_GlowStackPresent_AndBeatCoupled`（L-4 等价：glow 与节拍同源） | 已实现（样片面） |
...
| LT-9 | — | T3 待做（产物面：光数计入 C_light 档位判定需编译器报告） |
...
| ST-4 ~ ST-6 | — | T3 待做（风格越权检测需编译器参数合并链） |
| CT-1 | `TechniqueFamilyBoundaryGateTests.ShaderLibrary_AllShadersImport_AndCarryStyleStage`（cartoon 分支端口面） | 已实现（端口面；数值面 T3） |
| CT-2 ~ CT-8 | — | T3 待做（卡通数值谓词需截帧/产物面；T2c-C 质量收敛已按 STYLE_IMPL 2.8-1 阶梯环落地，见提交 242223dc） |
| PX-1 ~ PX-5 | — | T3 待做（像素维度样片属 T4 铺开；shader 侧量化机制已在库，`VfxStyleStage.hlsl`） |
...
| WL-4 | — | T3 待做（产物 GUID 命中白名单需编译器产物） |
| WL-5 | — R8013/E601 语义由 `VfxOutputAuditor` 沿用（旧根已换新），产物级测试属 T3 | T3 待做 |
| WL-6 | `AssetAllowListGateTests.WL6_TheAllowListIsOutsideEveryBuildWriteSurface` | 已实现 |
| CM-1 | — | T3 待做（编译报告 vs 实测一致性） |
...
| CM-3 | `ParadigmSampleGateTests.Sample_FitsThePmTierBudget`（分项结构预算样片面） | 已实现（样片面；产物面 T3） |
| CM-4 | — | T3 待做（六档接口一致性需六档产物） |
| CM-5 | — | T3 待做（W404 降级登记需编译报告） |
| CM-6 | `ParadigmSampleGateTests.SeedDerivation_IsDeterministic_AndDimensionSeparated`（确定性种子面）；产物字节级 sha 一致属 T3 | 已实现（种子面） |
...
| RT-5 | `ParadigmSampleGateTests.DissolveOut_ProgressInterface_AndEmptyTargetDegradation`（空引用退化确定行为） | 已实现（样片面；fallbackMode 声明面 T3） |
| RT-6 | `VfxControllerV2Tests.ResetForPool_RestoresTransform_AndPhaseState` + `ResetForPool_RestoresDebrisToKinematicInitialPose` | 已实现（VisualEffect 项随 GPU 粒子 T3） |
...
| RT-9 | — | T3 待做（编排内联需编排编译器） |
| BD-1 | `TechniqueFamilyBoundaryGateTests.BindingKeys_ResolveRoundTrip_AndUnknownRejected` | 已实现 |
| BD-2 | `VfxControllerV2Tests.ParameterBlock_DisabledLayerBinding_IsNoOp`（targetIndex=-1 空操作） | 已实现（变体清单声明面 T3） |
| BD-3 | `RuntimeAssemblyBoundaryTests`（Runtime 程序集零 UnityEditor 引用；System.Reflection 扫描属 T3 收严） | 已实现（部分面） |
...
| RF-2 | — | T3 待做（产物 Probe 接线） |
| RF-3 | — | T3 待做（编译报告声明） |
| RF-4 | — | T3 待做（Queue 偏移需产物） |
| RF-5 | — | T3 待做（2D 初值面需产物） |
...
| GA-6 | — | T3 待做（翻页交互 PlayMode 谓词） |
| GA-7 | — | T3 待做 |
| GA-8 | `T2cParadigmSampleCaptureTests`（PlayMode：LoadSceneInPlayMode 截帧走真实场景） | 已实现（PlayMode） |
...
豁免纪律（§7.5）：豁免项仍跑谓词（对应测试对豁免面做"跳过/命中"双向断言，如 PX-6 三分支）；fail-closed 三路不可豁免——本表 4 行豁免均不属于三路。
