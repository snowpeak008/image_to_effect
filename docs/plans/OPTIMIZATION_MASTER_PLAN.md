...
| **T2b 技术族实现规格 + 画廊** | 5 技术族在 URP 下的实现规格（Shader Graph 子图库、VFX Graph 模板、程序化网格生成器、局部光节拍器）；卡通/像素两风格的材质族设计；Recipe v2 编译器边界修订（识别组件、shader/vfx 资产准入、成本模型六档）；2D/3D 两个九宫格画廊场景（翻页/Play 即检，Bloom 可开关）；含 ADR-010 §4bis 八条勘注（折射可选层降级、UGUI 双方案、编排内联、像素体素感、低档材质暗边、辉光边界、参考图定位、新增 `radial_spike_array`） | **DONE**（2026-09-06 凌晨合并 `41d38e2d`。**规格线**：`d32c84a6` 交付 7 文档→用户合入 `1168e2c6`→初审 ACCEPTED，拍板 #8 `element.strength`/#11 Gallery 进仓/#16 PX-6 条件谓词。**代码线**：三任子 agent（`276e9e1e` 单元1后中断、`80d8c9bb` 停滞于嵌套派发、`33632b0f`（Fable 5）一次跑通 2a~5+微调），8 笔至 `01cc5874`——20 shader（HLSL 路线自裁，含 GlowStack §3bis 全参+耦合补偿、RefractDual 双路、StyleStage 卡通/像素）/20 网格生成器（含 radial_spike_array bimodal）/VfxLightBeat（5 波形双驱动光:烘 MPB 广播）/控制器 v2（零复用旧代码）/CPU 14 模板（flipbook 构建期禁）/Element+StylePreset 及 5 样例/边界 v2（识别闭集+112 绑定键单一实源 fail-closed）/门禁 158 测试（PX-6 条件式）/2D+3D 画廊进仓（Editor 脚本确定性生成，Bloom 可开关）。**双组审计**：机械 PASS 零越界（.NET 1106/0 实绿，EditMode 846=792过/0失败/54跳过复跑吻合，manifest 仅内置 cloth/screencapture）；语义 PASS-with-remarks，3 条建议已微调复绿（M-13 shader 内 `pow(size,k)` 补偿/绑定键 `VfxBindingKeys` 单一实源+双向测试/PX-6b Lit 分支占位）。**已知限制记 T2c 输入**：GPU .vfx 14 未产出（无 visualeffectgraph 包，引包属依赖裁定；CPU 14 承载参数面）/材质 8 变体 P1P2/AssetAllowList.json 未落盘/156 谓词编号未逐条映射/followTarget 未实现/台账 #12~15） |
| **T2c 范式样片 + 旧资产清理** | 3 原型 × 3 元素 × 1 风格，2D/3D 各一组，零贴图零序列帧；同批闭合清理旧 sprite 模板/recipe/Generated/预览/构建清单/仅服务旧资产的 Editor 与测试；AI.Providers 目录快照等 re-export | **DONE 待用户九格判定**（2026-09-06 傍晚合并 `fba3f762` 推送。两任子 agent（`2ccc1582` A/B/C 后中断、`bbe07f41` 续 D/E/F+微调），7 笔至 `dcfa968e`。交付：①18 样片（{shield,chain_link,dissolve_out}×{ice,lightning,poison}×cartoon×{2D,3D}，`VfxSampleAssembler` 通用输入面+确定性种子，PM 档，语义审计验证"换输入可产新组合，非硬编码"；接口真实接线：shield hitAt/integrity/break 12 块 Voronoi 破碎、chain_link onHop 逐跳、dissolve_out setProgress+空退化）；②画廊判定页（行=原型列=元素，B 键 Bloom）+ contact-sheet `docs/design/paradigm/t2c-review/`（7 节拍帧×双 Bloom+参考图标尺条+metrics.json，质量两轮收敛）；③旧范式清零（Templates 全树/旧 Generated 254 目录/Recipes/Preview 30 场景/BuildManifests 293/旧 Editor 13 目录+Runtime 30+/测试 115 文件；保留 VfxCompiler 回滚/W24 recorder/S5 gate/S6 worker；v1 管线空目录 E308 fail-closed）；④AI.Providers 快照 re-export 2.0.0（6 行 v1 形状投影指新变体，缺口如实登记留 AI 回路改造卡，AI.Tests 294/Desktop 187/Cli 142 pin 同步全绿）；⑤AllowList 落盘（vfxTemplates:[] fail-closed）+WL 双向测试/156 谓词登记表（口径 78 已实现/75 T3/3 豁免+自洽断言）/C_over 校准（系数维持）/VFX_LiquidBlobRefract。**双组审计 PASS-with-remarks 零阻塞**（机械复跑 EditMode 572/572/0、.NET 1106/0 吻合、删除面零越界零误删；语义验证防偏离/自包含/删除判定/AI 替代诚实性），3 条建议已微调 `dcfa968e`（报告 physics2d 失实修正/登记表口径/BuildSettings 悬空清理，门禁 34/34 绿）。**新基线：EditMode 572/0（0 跳过），.NET 1106/0**。六准则自检全过（2D 静帧各向异性 1.02 弱项如实登记 T3）。**用户九格判定 FAIL（2026-09-06 晚）**：视觉不合格（雾幕大色块糊屏/雷电裂纹全黑/整体发灰无亮度分级），结构层未否定。验收流程复盘：全链数值代理绿但无人看过像素——**新硬规矩：内容卡合并前主 agent 必须亲眼读渲染帧对比参考图**。→ **T2cv 视觉返工卡已派**（`6bae5810`，worktree `D:\wt\i2s-t2cv`）：V1 雾幕/V2 裂纹发光/V3 亮度质感/V4 重建+新 contact-sheet/V5 报告；强制逐轮截帧读图自评；修复限技术族通用面。T2cv 过主 agent 肉眼关后再送用户复判；复判通过 → T3 |
