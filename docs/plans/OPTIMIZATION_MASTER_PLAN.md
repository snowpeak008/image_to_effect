...
| F6 | IN PROGRESS | 主 agent（崩溃恢复续接） | F2 已合入解锁。**崩溃恢复（2026-08-30）**：F6 子 agent 在 Cursor 崩溃前的未提交 WIP 从 worktree `D:\wt\i2s-f6` 找回，补齐配套断言后转绿，提交至分支 `task/F6-e2e-acceptance`（未合 master、未审计）：`a23c6b6c` 落必做①③④⑥（EntryLabel ItemId、入口装配体 IL 越界面扫描、MCP/生产组合根零网络实测、VFXBxxxx 失败码以 `failure:` artifact 落队列可见面 + CLI/MCP artifactIds 展开）；`fea2577e` 落必做⑤（batches/ 样例清单 + JSON schema + 解析器守护测试，prompt-kind）；`ed915dbb` 落必做②（两侧投影补齐 itemId/cancelRequested + JobEntryProjectionEquivalenceTests 反射 JobRecord 构造性守护）；`02c0cf13` 落流程二 E2E（入库样例经真实 CliRunner 走完 run→报告→退出码 + `eng/run-f6-e2e-acceptance.ps1` 人工验收脚本）。worktree 全量 Release **744/744** 0/0。必做①-⑥ 全部落地。**未做**：⑤ 的 strict-budget recipe-kind 样例、流程一真机 E2E（需 Unity 2022.3.62f3c1 Editor；用户处 `spike/image_to_smart` 恰为该版本工程，可作真机验收目标；本机仅 Hub 无 Editor）、裁量项。**续接**：装 Editor 后跑流程一真机 batchmode + strict recipe 样例，独立审计后合并 master。备注：全解并行跑偶发 1 例 MCP stdio 时序 flake（既有、单跑必绿） |
