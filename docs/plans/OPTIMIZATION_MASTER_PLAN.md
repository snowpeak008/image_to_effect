...
| **T2a 原型目录 v1 + Recipe v2 schema 草案** | 系统性穷举 Unity 可表达的特效空间 → 原型目录（预计 40~60，每原型一页：职责/层结构/节拍/2D-3D 差异/对外接口/六档降级）+ 元素目录 + 风格目录（卡通/像素首批）+ Recipe v2 schema 草案（层/族/三轴/档位/接口）。**用户只审分类与数量**。docs-only | **派发中** |
| **T2b 技术族实现规格 + 画廊** | 5 技术族在 URP 下的实现规格（Shader Graph 子图库、VFX Graph 模板、程序化网格生成器、局部光节拍器）；卡通/像素两风格的材质族设计；Recipe v2 编译器边界修订（识别组件、shader/vfx 资产准入、成本模型六档）；2D/3D 两个九宫格画廊场景（翻页/Play 即检） | 待 T2a 用户审 |
