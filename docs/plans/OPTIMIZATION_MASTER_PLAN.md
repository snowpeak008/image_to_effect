...
| **T2b 技术族实现规格 + 画廊** | 5 技术族在 URP 下的实现规格（Shader Graph 子图库、VFX Graph 模板、程序化网格生成器、局部光节拍器）；卡通/像素两风格的材质族设计；Recipe v2 编译器边界修订（识别组件、shader/vfx 资产准入、成本模型六档）；2D/3D 两个九宫格画廊场景（翻页/Play 即检，Bloom 可开关）；含 ADR-010 §4bis 八条勘注（折射可选层降级、UGUI 双方案、编排内联、像素体素感、低档材质暗边、辉光边界、参考图定位、新增 `radial_spike_array`） | **规格线 DONE / 代码线续做中**（规格：旧会话 docs-only 子 agent `d32c84a6` 交付 7 文档，用户合入 `1168e2c6`，主 agent 初审 ACCEPTED，用户拍板 #8 strength/#11 Gallery 进仓/#16 PX-6 条件谓词；代码：worktree `D:\wt\i2s-t2b` 单元1已交 `a9eb0d4a`，单元2 Shader WIP 中断，待重派续做单元2~5，交付报告命名 `T2B_IMPL_REPORT.md`） |
