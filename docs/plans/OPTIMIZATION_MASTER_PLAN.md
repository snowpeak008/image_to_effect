...
| **F8b2 草稿 store 版本链**（**IN PROGRESS**，2026-09-02 派发开发子 agent（Fable 5 High），worktree `D:\wt\i2s-f8b2`、分支 `task/F8b2-draft-store-lineage`、基于 `8428199d`。主 agent 派发时定版：①`RecipeDraftRecordFormatVersion` 1→2，版本 1/未知 fail-closed 且新增 `UnsupportedVersion` 与 `StorageFailed` 区分；②§7.2 七个新字段 + `Superseded` 终态；③回退截断 API，审计记录阻止截断；④两级 cap 与受保护记录拒新版，读取上界 ≤32 MiB 且有满额 store 测试；⑤store 层禁止静默丢弃——写入 API 返回类型化 trim 结果，Desktop 提示/时间线接线归 F8b3；⑥**RG-6 裁决：durable 跨进程锁**（仿 `ProviderConfigurationRevisionLock`，超时 fail-closed 稳定码）；⑦AI 首版 origin=ai_draft、F8a2 示例卡 origin=preset+presetId 落地；⑧API 加法优先，机械适配逐一报告；⑨CLI/MCP backlog 语义回归、不因 origin 拒构建、不新增命令/工具） | schema 升版（仿 F3c）：lineage/origin/parentDraftId、两级 cap、trim 可见语义；跨入口回归（CLI/MCP 构建取草稿路径） | R5 |
