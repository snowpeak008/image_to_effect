...
| F6 | IN PROGRESS | 主 agent（崩溃恢复续接） | F2 已合入解锁。**崩溃恢复（2026-08-30）**：F6 子 agent 在 Cursor 崩溃前的未提交 WIP 从 worktree `D:\wt\i2s-f6` 找回，补齐配套断言后转绿，提交至分支 `task/F6-e2e-acceptance`（未合 master、未审计）：`a23c6b6c` 落必做①③④⑥（EntryLabel ItemId、入口装配体 IL 越界面扫描、MCP/生产组合根零网络实测、VFXBxxxx 失败码以 `failure:` artifact 落队列可见面 + CLI/MCP artifactIds 展开）；`fea2577e` 落必做⑤（batches/ 样例清单 + JSON schema + 解析器守护测试，prompt-kind）。worktree 全量 Release **739/739**（较基线 +6）0/0。**未做**：必做②（CLI/MCP 双面等价性构造化反射测试）、⑤ 的 strict-budget recipe-kind 样例、E2E 两条主流程（流程一需真实 Unity batchmode，本机 Hub 在但未装 2022.3.62f3c1 Editor）、人工验收脚本、裁量项。**续接**：先补②再搭 E2E 骨架（含 strict recipe 样例），独立审计后合并 master |
