...
| **T2c 范式样片 + 旧资产清理** | 3 原型 × 3 元素 × 1 风格，2D/3D 各一组，零贴图零序列帧；同批闭合清理旧 sprite 模板/recipe/Generated/预览/构建清单/仅服务旧资产的 Editor 与测试；AI.Providers 目录快照等 re-export | **已派出**（2026-09-06 上午，子 agent `2ccc1582`（Fable 5），worktree `D:\wt\i2s-t2c` 分支 `task/T2c-paradigm-samples` 基于 `6e4a477e`，feed 已复制。主 agent 三裁定：GPU .vfx 引包留 T3（CPU 粒子承载）/ 样片入库路径按画廊 `ResolveCellPrefabPath` 约定 / 样片用 Editor 脚本确定性组装。六单元：A=3D 九格+构造器 B=2D 九格 C=截帧对比入库 D=旧资产全清（先立后删，AI.Providers 最小一致替代）E=T2b 遗留（AllowList 落盘/156 谓词登记表/C_over 校准/liquid_blob 折射槽）F=报告。质量自检=REFERENCE_ANALYSIS §2 六准则。主仓 19 条漂移已清（用户关 Unity 后按还原纪律）） |
