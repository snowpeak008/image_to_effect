...
| **T2b 技术族实现规格 + 画廊** | 5 技术族在 URP 下的实现规格（Shader Graph 子图库、VFX Graph 模板、程序化网格生成器、局部光节拍器）；卡通/像素两风格的材质族设计；Recipe v2 编译器边界修订（识别组件、shader/vfx 资产准入、成本模型六档）；2D/3D 两个九宫格画廊场景（翻页/Play 即检，Bloom 可开关）；含 ADR-010 §4bis 八条勘注（折射可选层降级、UGUI 双方案、编排内联、像素体素感、低档材质暗边、辉光边界、参考图定位、新增 `radial_spike_array`） | **已派出**（2026-09-05 深夜，开发子 agent `276e9e1e`，模型 **Fable 5**——可用清单无 Opus 5 High，照实登记；worktree `D:\wt\i2s-t2b`，分支 `task/T2b-technique-families`，基于 `3156863b`，feed 已复制；派发前小任务试探通过。任务书五单元：①Recipe v2 schema 定稿+变体库清单（§4bis 裁定落地、`element.strength` 改名、`radial_spike_array`、光晕参数面入层参数）②Shader Graph 子图库+StyleStage 卡通/像素+光晕层+折射可选层降级 ③18+1 网格生成器+局部光节拍器+运行时控制器 v2 ④VFX Graph 场模板+ElementPreset/StylePreset 格式与样例 ⑤编译器边界修订+技术族门禁+2D/3D 画廊（Bloom 可开关）。红线四条（禁序列帧/资产外不碰/单一特效即偏离/只依赖 Unity）与基线（.NET 1106、EditMode 688）已写入任务书。**另发现旧会话遗留的 docs-only 规格子 agent `d32c84a6` 仍在运行**（上一会话实际派出但快照漏登记）：只在主仓 `docs/design/paradigm/` 新增 5 技术族规格文档（MATERIAL 已落成，PARTICLES 进行中），禁 git 写操作——与代码线互补，验收时做一致性对照，冲突以代码线实测为准） |
