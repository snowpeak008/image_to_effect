...
2. Desktop 不直接读写 Unity 项目文件；项目写入只允许经批准的构建路径，且写入范围限定 `Assets/VFX/Generated`（与 `VfxCompiler.GeneratedRoot` 一致）；共享资产目录 `Assets/VFX/Shared` 的写入政策由 ADR-007 裁决，裁决前一律视为只读。
