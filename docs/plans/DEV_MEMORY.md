...
> **已办**：master 已推送；i2s-t2b 已退役；主仓 19 条漂移已清（用户关 Unity 后）。**T2c 已派出**（2026-09-06 上午，`2ccc1582` Fable 5，worktree `D:\wt\i2s-t2c` 基于 `6e4a477e`；三裁定：GPU 引包留 T3/样片路径按画廊约定/Editor 脚本组装；六单元 A~F 见状态板）。原 T2c 要点——样片 {shield, chain_link, dissolve_out} × {ice, lightning, poison} × cartoon 2D/3D 各 9 格 + 旧资产全清同批闭合 + T2b 遗留输入（GPU .vfx 引包裁定/8 变体 P1P2/AssetAllowList 落盘+WL 双向测试/156 谓词编号登记表/followTarget/台账 #12~15 校准）。T2c 派卡前写覆盖面声明（ADR-010 §10-2）。
