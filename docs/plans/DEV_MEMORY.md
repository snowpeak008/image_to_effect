...
> **待办**：①推送 master；②退役 worktree `D:\wt\i2s-t2b`（分支已合并可删）；③主仓 19 条 project/ 漂移等用户关 Unity 后清；④**T2c 解锁**——样片 {shield, chain_link, dissolve_out} × {ice, lightning, poison} × cartoon 2D/3D 各 9 格 + 旧资产全清同批闭合 + T2b 遗留输入（GPU .vfx 引包裁定/8 变体 P1P2/AssetAllowList 落盘+WL 双向测试/156 谓词编号登记表/followTarget/台账 #12~15 校准）。T2c 派卡前写覆盖面声明（ADR-010 §10-2）。
