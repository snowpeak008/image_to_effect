...
> **验收流程缺陷复盘（已立规矩）**：T2c 全链（测试→数值自评→双组审计→合并）没有任何一环真正看过渲染像素——数值代理全绿但肉眼不合格。**新硬规矩（内容卡调度纪律，2026-09-06 用户批评后定版）**：①合并前主 agent 必须亲眼读渲染帧并与参考图并排对比，不过关即打回，不送用户验收；②**监控内容卡不得只看提交数/脏文件数等过程信号——每次轮询须抽读子 agent 落盘的中间截帧**，视觉跑偏当轮打断纠偏，不等交付；③内容卡任务书必须内置"改→截帧→读图自评→再改"循环（代码卡的"测试绿=完成"判据对美术内容无效，像素才是 ground truth）。根因复盘：主 agent 此前只有代码卡调度经验（测试为真值、审计为闸），把美术卡按代码卡剧本调度是本次事故的直接原因。
