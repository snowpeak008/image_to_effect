...
> **T2cv 已派**（一派 `6bae5810` 于 19:35 用户断网时在勘察阶段中断，零提交零 WIP；**重派 `bc79a332`**（Fable 5），任务书加两条：中间截帧逐轮编号入库供主 agent 抽读、自评仅用于迭代不作验收证据；worktree `D:\wt\i2s-t2cv` 基于 `8f46de41`）：V1 雾幕修复/V2 裂纹发光/V3 亮度分级+质感/V4 重建 18 样片+新 contact-sheet 入库 `t2cv-review/`/V5 报告含逐轮看图迭代记录。任务书强制"改→截帧→子 agent 自己读图自评→再改"循环，修复限定技术族/构造器通用面（禁逐格特判）。完成后主 agent 亲眼看图验收，过关才送用户复判。
