...
> **CURRENT CLOSEOUT — U6 FINAL GO / A0 AI ENDPOINT REBASE (2026-08-29).** ADR-005's ordinary-user Phase-2 route is complete at `100/100`; ADR-006 defines the separate post-U6 AI chain with user-owned opaque endpoints. The pre-U0 report below remains historical evidence only; its privileged route and blockers are not current delivery dependencies.
>
...
In this same documentation rebase, A0 remains `CLOSED — AI_PROVIDER_TWO_CHANNEL_ROUTING`. ADR-006 changes the endpoint contract without changing implementation bytes, provider behavior, credential, Desktop UI, Broker, Worker, Unity, project path, or runner behavior. It relies only on the accepted U6 unified-gate evidence and does not rerun the project gate; its validation is `git diff --check`.

The formal new DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A1 remains the sole `ACTIVE` package; A2 through A6 are `NOT STARTED`. The earlier A1 `NO-GO — P0/P1/P2=0/0/1` is superseded by the user product decision, not treated as a GO. A1 must replace endpoint admission with `OpaqueEndpoint`, then revalidate contracts, schema/store/import/resolver/redaction and the Gateway boundary; no real Chat or Image HTTP is implemented.

ADR-006 freezes channel routing: all LLM/conversation work must use the one explicit `ChatLlm` binding and all image generation must use the one explicit `ImageGeneration` binding. Each is exactly one explicit profile/capability/model; sharing a profile is permitted only through separate capabilities and separate bindings. Origin (`Official`, `Relay`, `Friend`, `Subscription`, `Custom`) is metadata only; explicit protocol remains independent. Missing, invalid, cross-channel, or failed binding state is fail-closed with no fallback. A user endpoint is an opaque configuration string: official, relay, friend, subscription, and custom addresses all save and resolve locally without endpoint syntax or upstream-validity admission.

The frozen operational boundary keeps `SecretRef`/DPAPI CurrentUser as the recommended formal key store, no cookie scraping/scripts/custom-header templates/dynamic DLL/TLS bypass, no secrets or AI network in Broker/Worker/Unity, and Desktop through Gateway only. Schema and resolver acceptance are local persistence facts, never network authorization. A2/A3 adapters will attempt to interpret the saved endpoint only at request time; construction/network/upstream failure must return a stable redacted error, make no configuration write-back, and select no fallback. Logs, exceptions, receipts, ordinary UI, and default exports exclude raw endpoints (including query/user-info), keys/Auth/prompt/raw/base64 data; only an explicit edit surface may show the user value, and explicit configuration export requires a warning. Tom import is user-confirmed, preserves endpoint text as opaque draft data, and never decrypts `ApiKeyProtected`; images remain in private cache and are never automatically written to Unity.
