...
The formal new DAG remains `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A4 `AI_DESKTOP_WIRING` is `CLOSED — FINAL GO — P0/P1/P2=0/0/0` at commits `fc986d11`, `ffc9f609`, and `cc5ff806`. Its final receipt is `186/186`; AI is `77/77`, Desktop `22/22`, and the total is `423/423`; frozen-root replay and owned residue are `0`. A5 `AI_MOCK_E2E` is `CLOSED — FINAL ACCEPTED / GO — P0/P1/P2=0/0/0` at `9152c7e6` plus `14abb1d3`, merged at `c6f9920f`; receipt `a5-final-acceptance-14abb1d3-bootstrap` records real-loopback `11/11`, `209` hashes, unchanged tracked locks, and residue `0`. A6 `WP-AI-PROVIDER-FINAL-AUDIT` is `CLOSED — FINAL ACCEPTED/GO — P0/P1/P2=0/0/0`; all A0–A6 are closed and ADR-006 AI scope is accepted at `100/100`.

ADR-006 freezes channel routing: all LLM/conversation work uses one explicit `ChatLlm` binding and all image generation uses one explicit `ImageGeneration` binding. Each channel has independent explicit profile, secret, capability/model, and protocol; missing, invalid, cross-channel, or failed state is fail-closed, with no fallback. Origin (`Official`, `Relay`, `Friend`, `Subscription`, `Custom`) is metadata only. `OpaqueEndpoint` is arbitrary bounded user text—including empty and whitespace values—saved and resolved exactly, with interpretation only at request time; local configuration acceptance is never request authorization.

A4 closed within its former exact Desktop/AI allow-list and its result preserves zero automatic network traffic, opaque endpoint storage, `SecretRef`/DPAPI entry-only settings, explicit revoke fail-closed behavior, no fallback, redaction, promptly closed preview streams, and zero Desktop project writes. The A4 record is component evidence only and does not claim local mock-handler cross-channel E2E.

A5's closed all-and-only implementation scope was:
...
The existing `ProviderDesktopRuntime` constructor remains. Its only A5 production change is the optional `privateImageTempRoot` parameter transparently passed to `ImageGateway`; production passes `null` exactly as before. A5 executed a real loopback `TcpListener` via the production runtime and production `HttpClient` handlers—never an injected handler, external endpoint, or paid call.

The A5 flow is Settings CRUD/DPAPI/explicit channel bindings -> Create Chat -> Preview Image, with both base64 and URL image responses decoded by the existing decoder. It includes restart persistence, channel isolation, opaque endpoints, failures, revoke/fail-closed behavior, redaction, private-artifact cleanup, and no project write. Its accepted receipt records root replay `0`, locked Release build, `11/11` A5 test, product-assembly binding, `209` receipt hashes, unchanged tracked locks, and `vfxcomposer-a5` residue `0`; a root/build/test/binding/residue failure, scope drift, changed production `null` behavior, handler injection, external/paid traffic, fallback, raw-data leak, or project write remains STOP.

A6 `WP-AI-PROVIDER-FINAL-AUDIT` is `CLOSED — FINAL ACCEPTED/GO — P0/P1/P2=0/0/0`. The accepted Git-ignored receipt is `.codex_tmp/a6-ai-provider-final-audit-20260829T000000Z/audit-summary.json`: independent gate `434/434`, schemas `23`, source stable/root `0`/residue `0`, empty stdout with exact Broker `W24FS001` stderr and exit `23`, approved package feed `39/39`, `18` unique ignored locks, and tracked locks unchanged. It completes the A0–A5 read-only audit without repair or product-gate rerun. The result formally accepts the ADR-006 dual-channel scope at `100/100`: Chat is the sole binding for all LLM/conversation requests; Image is the sole binding for all image-generation requests; profiles, secrets, and protocols are independent; and no fallback exists. `OpaqueEndpoint` remains arbitrary bounded text including empty/whitespace, with request-time-only interpretation. It is not real paid-provider authentication or a production release. No AI work package remains active; future requirements must start a new milestone.
