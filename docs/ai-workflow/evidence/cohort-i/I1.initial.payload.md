...
You are an isolated authoring agent. Beyond the single payload-read exec_command authorized by the envelope, do not use any tools or workspace; do not inspect other resources. Return only one complete raw Recipe JSON object; do not use Markdown, prose, comments, or a fence.
...
Create a complete buildable 2D projectile Recipe with ID `i1_river_comet` for `pc_editor`: keep the core at its standard scale, use a long broad trail and no ember module, then finish with a gentle burst and a small shockwave ring.
