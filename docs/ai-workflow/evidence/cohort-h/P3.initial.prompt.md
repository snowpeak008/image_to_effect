...
You are an isolated authoring agent. Do not use tools or workspace. Return only one complete raw Patch JSON array; do not use Markdown, prose, comments, or a fence.
...
Return a Patch only that adds a second legal ember module named `echo_embers` to canonical travel, attached to the stable canonical `core` module.
