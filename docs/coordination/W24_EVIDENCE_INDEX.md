...
> **CURRENT EVIDENCE ROUTING — U6 FINAL GO / A0–A3 CLOSED / A4 SOLE ACTIVE (2026-08-29).** ADR-005 closes the USER_MODE route at `100/100`; ADR-006 controls the separate post-U6 AI route with user-owned opaque endpoints. A2 and A3 have final GO evidence, and A4 Desktop wiring is the only active package. The pre-U0 entries retained below preserve exact history but cannot be selected as proof of the ordinary-user runtime or treated as current dependencies/blockers.
>
...
- The formal AI DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A0-A3 are closed; A2/A3 are each final GO at `P0/P1/P2=0/0/0`, A4 `AI_DESKTOP_WIRING` is the sole `ACTIVE` package, and A5-A6 are `NOT STARTED`. This post-U6 chain is excluded from the closed USER_MODE `100/100` accounting.
...
### U6 final-audit closeout, A1–A3 final acceptance, and A4 evidence contract
...
`A4 — AI_DESKTOP_WIRING` is the sole active package and must produce the next evidence. It may add only the three `AI.{Contracts,Providers,Tests}/Desktop/**` roots, `PrivateImagePreviewDecoder.cs`, and `AiDesktopIntegrationTests.cs`; it has the exact resolver/secret-store/Chat/test overlays, Desktop csproj/lock/App startup/view-model/view scope, `NoProjectAccessSurfaceTests.cs`, Desktop.Tests csproj/lock, and runner/baseline allowance recorded in the Registry. It must stop rather than touch Client, Broker, Worker, Unity, any project or solution path, `OpenAiCompatibleImageGateway`, or `MainWindow.axaml`.

The A4 evidence matrix must show that save, startup, and Create/Settings/Preview navigation send zero network traffic; `Unknown` health permits an explicit prompt, whose result alone records health; Image has neither automatic health nor automatic paid request; settings secret re-entry is entry-only and explicit revoke clears the selected `SecretRef` and fails closed; no route/credential/model/protocol/endpoint/channel fallback occurs; and ordinary UI/diagnostics/export redact endpoint sensitive parts, secrets/auth, prompts, raw payloads, and image bytes. It must also show that `PrivateImagePreviewDecoder` accepts only a provider-issued `Stream`, creates an in-memory Avalonia `Bitmap`, and promptly closes the stream without `File`, `Directory`, `Path`, `FileStream`, `Environment`, `System.Net`, project, or Unity use.

A4's focused fake/spy UI and surface tests may substantiate those local controls, but mock-handler cross-channel E2E is reserved for A5. A4 may publish only with its exact runner/baseline integration, targeted AI/Desktop tests, locked Release solution build, redaction/forbidden-surface scan, `git diff --check`, and clean worktree. A5 and A6 remain `NOT STARTED`.
