...
> **CURRENT EVIDENCE ROUTING — U6 FINAL GO / A1 FINAL ACCEPTED / A2+A3 ACTIVE (2026-08-29).** ADR-005 closes the USER_MODE route at `100/100`; ADR-006 controls the separate post-U6 AI route with user-owned opaque endpoints. A1 has final accepted evidence, while A2 and A3 are the only active, disjoint channel packages. The pre-U0 entries retained below preserve exact history but cannot be selected as proof of the ordinary-user runtime or treated as current dependencies/blockers.
>
...
- The formal AI DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A0 and A1 are closed; A2 `WP-AI-CHAT-CHANNEL` and A3 `WP-AI-IMAGE-CHANNEL` are the only concurrent `ACTIVE` packages; A4-A6 are `NOT STARTED`. This post-U6 chain is excluded from the closed USER_MODE `100/100` accounting.
...
### U6 final-audit closeout, A1 final acceptance, and A2/A3 evidence contracts
...
The final A1 evidence is AI tests `23/23 × 3`, schema opaque-endpoint vectors `9`, and independent gate receipt `D:\wt\i2s-a1\.codex_tmp\a1-phase2-gate-092b7d6b3aeb4246928688323771e8b8`: self-excluded receipt manifest `167/167`, Release solution `0 warnings / 0 errors`, frozen-root replay `0` mismatches, and point-in-time runtime-process/named-pipe/owned-temp-root residue `0`. This establishes `A1 CLOSED GO`; no active channel package may change an A1-owned path.

`A2 — WP-AI-CHAT-CHANNEL` is `ACTIVE` with exactly `src/VFXComposer.AI.Contracts/Chat/**`, `src/VFXComposer.AI.Providers/Chat/**`, and `src/VFXComposer.AI.Tests/Chat/**`. It interprets the selected protocol only at request time and sends the stored full endpoint URL unchanged, without normalization, modification, path append, or concatenation. Its permitted explicit protocol shapes are OpenAI Chat/Responses, Anthropic Messages, Gemini GenerateContent, and OpenAI-compatible. Auth is per request only, never `DefaultHeaders`; there is no fallback; cancellation, timeout, `429`, malformed request/response, parser, network, and upstream failures are typed and redacted; automated transport tests use mock handlers only.

`A3 — WP-AI-IMAGE-CHANNEL` is concurrently `ACTIVE` with exactly `src/VFXComposer.AI.Contracts/Image/**`, `src/VFXComposer.AI.Providers/Image/**`, and `src/VFXComposer.AI.Tests/Image/**`. It permits only OpenAI Images-compatible request-time transport at the stored full endpoint exactly as supplied. It normalizes `b64` and URL results. URL download never forwards authorization and must enforce redirect, MIME, byte, dimension, and hash limits before retaining a private temporary artifact. It must not write Unity/project state, auto-retry, or add paid CI.

The A2/A3 root sets are zero-overlap: neither may edit a common file, `.csproj`, lock, solution, runner, or baseline. Existing SDK globs must already compile and test each owned subtree; an inadequate glob is STOP. Each package must retain targeted-test and locked Release solution-build evidence, but neither updates the runner/baseline or independently runs the gate; A4 owns the first unified runner/baseline/gate integration. A4-A6 remain `NOT STARTED`.
