...
> **CURRENT EVIDENCE ROUTING — U6 FINAL GO / A0–A4 CLOSED / A5 SOLE ACTIVE (2026-08-29).** ADR-005 closes the USER_MODE route at `100/100`; ADR-006 controls the separate post-U6 AI route with user-owned opaque endpoints. A2, A3, and A4 have final GO evidence, and A5 local mock E2E is the only active package. The pre-U0 entries retained below preserve exact history but cannot be selected as proof of the ordinary-user runtime or treated as current dependencies/blockers.
>
...
- The formal AI DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A0-A4 are closed; A2/A3/A4 are each final GO at `P0/P1/P2=0/0/0`, A5 `AI_MOCK_E2E` is the sole `ACTIVE` package, and A6 is `NOT STARTED`. A4's record is commits `fc986d11`/`ffc9f609`/`cc5ff806`, receipt `186/186`, AI `77/77`, Desktop `22/22`, `423/423` total, root/residue `0`. This post-U6 chain is excluded from the closed USER_MODE `100/100` accounting.
...
### U6 final-audit closeout, A1–A4 final acceptance, and A5 evidence contract
...
A5 is the sole active evidence producer. Its implementation scope is exactly `tests/VFXComposer.AiLocalE2E.Tests/**`, `src/VFXComposer.AI.Providers/Desktop/ProviderDesktopRuntime.cs`, `VFXComposer.sln`, `eng/run-phase2-gate.ps1`, and `eng/phase2-baseline-roots.json`. `ProviderDesktopRuntime.cs` is the only production seam: preserve the existing constructor and add only optional `privateImageTempRoot` passed through to `ImageGateway`; production `null` behavior is unchanged.

The required A5 evidence is a real loopback `TcpListener` test through production runtime and production `HttpClient` handlers, with no handler injection, external network, or paid provider. It exercises Settings CRUD/DPAPI/explicit bindings -> Create Chat -> Preview Image, for both base64 and URL image responses through the existing decoder. The evidence must include restart persistence, channel isolation, opaque endpoints, failures, revoke/fail-closed behavior, redaction, private-artifact cleanup, and no project write.

The gate evidence must add root replay, Release build, A5 test, product-assembly binding, and `vfxcomposer-a5` residue proof. Any root/build/test/binding/`vfxcomposer-a5` residue failure, scope drift, changed production `null` behavior, injected handler, external/paid call, fallback, raw-data leak, failed cleanup, or project write invalidates the package. A6 remains `NOT STARTED` until A5 is frozen.
