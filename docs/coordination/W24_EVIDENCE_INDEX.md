...
> **CURRENT EVIDENCE ROUTING — U6 FINAL GO / A0–A5 CLOSED / A6 SOLE ACTIVE (2026-08-29).** ADR-005 closes the USER_MODE route at `100/100`; ADR-006 controls the separate post-U6 AI route with user-owned opaque endpoints. A2–A5 have closed package evidence; A6 is the only active, read-only audit package, and AI functionality final GO remains pending. The pre-U0 entries retained below preserve exact history but cannot be selected as proof of the ordinary-user runtime or treated as current dependencies/blockers.
>
...
- The formal AI DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A0-A5 are closed; A2/A3/A4 are each final GO and A5 `AI_MOCK_E2E` is `CLOSED — FINAL ACCEPTED / GO`, all at `P0/P1/P2=0/0/0`. A5 accepted source is `9152c7e6` plus `14abb1d3`, merged at `c6f9920f`; receipt `a5-final-acceptance-14abb1d3-bootstrap` records real-loopback `11/11`, `209` hashes, unchanged tracked locks, and residue `0`. A6 `WP-AI-PROVIDER-FINAL-AUDIT` is the sole `ACTIVE` read-only package; AI functionality final GO is pending its conclusion. A4's record remains commits `fc986d11`/`ffc9f609`/`cc5ff806`, receipt `186/186`, AI `77/77`, Desktop `22/22`, `423/423` total, root/residue `0`. This post-U6 chain is excluded from the closed USER_MODE `100/100` accounting.
...
### U6 final-audit closeout, A1–A5 final acceptance, and A6 evidence contract
...
A6 `WP-AI-PROVIDER-FINAL-AUDIT` is now the sole active evidence consumer. It must audit the entire frozen A0–A5 chain read-only: two-channel routing/no fallback; opaque configuration/request-time parsing separation; secret and sensitive-data redaction; no Broker/Worker/Unity provider-network role; real-loopback/no-paid-external-call evidence; frozen gates and tracked locks; and cleanup/residue. It has no product development, repair, implementation, or product-gate rerun authority. It reports any finding as STOP without repair; AI functionality final GO must not be claimed before A6 completes.
