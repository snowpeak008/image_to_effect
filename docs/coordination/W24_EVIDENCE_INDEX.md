...
> **CURRENT EVIDENCE ROUTING — U6 FINAL GO / A0 AI ENDPOINT REBASE (2026-08-29).** ADR-005 closes the USER_MODE route at `100/100`; ADR-006 controls the separate post-U6 AI route with user-owned opaque endpoints. The pre-U0 entries retained below preserve exact history but cannot be selected as proof of the ordinary-user runtime or treated as current dependencies/blockers.
>
...
- A0 `AI_PROVIDER_TWO_CHANNEL_ROUTING` is `CLOSED — DOCS ONLY`. This endpoint-decision rebase edits exactly seven documents: (1) `docs/rules/ADR-006_AI_PROVIDER_TWO_CHANNEL_ROUTING.md`; (2) `docs/stage-notes/POST_U6_AI_PROVIDER_TWO_CHANNEL_PLAN.md`; (3) `docs/stage-notes/W24_STANDALONE_DESKTOP_PHASE_PLAN.md`; (4) `docs/stage-notes/W24_STANDALONE_DESKTOP_PHASE2_REPORT.md`; (5) `docs/coordination/W24_PROGRAM_CONTROL.md`; (6) `docs/coordination/W24_WORK_PACKAGE_REGISTRY.md`; and (7) this evidence index. It has no implementation receipt and does not rerun the project gate; it consumes only the existing accepted U6 unified-gate evidence plus `git diff --check`.
- The formal AI DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A1 remains the sole `ACTIVE` package; A2-A6 are `NOT STARTED`. The prior A1 `NO-GO — P0/P1/P2=0/0/1` is superseded by the user endpoint decision, so A1 must rework/revalidate rather than claim closure. This post-U6 chain is excluded from the closed USER_MODE `100/100` accounting.
...
### U6 final-audit closeout and A1 evidence contract
...
The A0 evidence consists only of the seven-document endpoint-decision rebase listed above. It does not rerun the project gate; this commit must pass `git diff --check` and remain clean after its single commit. The fixed Tom design input is [`snowpeak008/Tom_doc@dd0f9ffc32d426735f7fb8960640e9b7ae9337bf`](https://github.com/snowpeak008/Tom_doc/tree/dd0f9ffc32d426735f7fb8960640e9b7ae9337bf), used only for non-sensitive import design.

`A1 — AI_PROVIDER_FOUNDATION` remains the sole active package and owns only `src/VFXComposer.AI.Contracts/**`, `src/VFXComposer.AI.Providers/**`, `src/VFXComposer.AI.Tests/**`, `docs/schemas/desktop/vfxcomposer-ai-provider-config-v1.schema.json`, `VFXComposer.sln`, `eng/verify-phase2-schemas.py`, `eng/run-phase2-gate.ps1`, and `eng/phase2-baseline-roots.json`. The verifier is the only additional file beyond the three roots, schema, solution, runner, and baseline. All other paths are forbidden. A central package-management or external `.csproj` dependency is STOP, not a reason to widen ownership. Its former `NO-GO — P0/P1/P2=0/0/1` is superseded by this product decision: it must replace endpoint admission with `OpaqueEndpoint` and revalidate.

A1 evidence must prove canonical configuration; migration/future-version rejection; corrupt primary/backup fail-closed recovery; DPAPI no-plaintext and unreadable payload; capability/channel mismatch; zero fallback; Tom `ApiKeyProtected` exclusion; Contracts/Providers/Tests and Broker/Worker/Unity/Desktop boundary; and redaction. It must prove `OpaqueEndpoint` as an arbitrary bounded user string: schema/store/resolver accept it unchanged without syntax/scheme/host/port/user-info/query/fragment/upstream-validity admission, while structural/type/version/size failures still fail. Such acceptance must invoke no network and never mean network authorization. A2/A3 later attempt request construction only at call time; parser/network/upstream failure is stable and redacted, writes nothing back, and falls back nowhere. `SecretRef`/DPAPI remains recommended but endpoint-embedded credentials do not reject a profile. Logs, exceptions, receipts, ordinary UI, and default exports redact raw endpoint values; only explicit editing and warned explicit configuration export may reveal them. A1 still provides atomic JSON/`.bak`, fingerprint, registry skeleton, safe Tom draft import, and `IAiGateway` without real Chat or Image HTTP; image output, if added later, is private cache only and never automatically written to Unity.

The first A1 writer is `STOPPED — ZERO WRITES` until the separate implementation rework starts. The rework must retain schema structural validation and prove endpoint storage bounds without reintroducing endpoint-format acceptance as a runtime gate; merely weakening counts or claiming local acceptance authorizes a request is STOP.
