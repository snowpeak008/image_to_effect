...
> **CURRENT EVIDENCE ROUTING — U6 FINAL GO / A0 AI ARCHITECTURE FREEZE (2026-08-28).** ADR-005 closes the USER_MODE route at `100/100`; ADR-006 controls the separate post-U6 AI route. The pre-U0 entries retained below preserve exact history but cannot be selected as proof of the ordinary-user runtime or treated as current dependencies/blockers.
>
...
- A0 `AI_PROVIDER_TWO_CHANNEL_ROUTING` is `CLOSED — DOCS ONLY` in this same single commit. It owns exactly six documents: (1) `docs/rules/ADR-006_AI_PROVIDER_TWO_CHANNEL_ROUTING.md` (new); (2) `docs/stage-notes/W24_STANDALONE_DESKTOP_PHASE_PLAN.md`; (3) `docs/stage-notes/W24_STANDALONE_DESKTOP_PHASE2_REPORT.md`; (4) `docs/coordination/W24_PROGRAM_CONTROL.md`; (5) `docs/coordination/W24_WORK_PACKAGE_REGISTRY.md`; and (6) this evidence index. A0 has no implementation receipt and does not rerun the project gate; it consumes only the existing accepted U6 unified-gate evidence plus `git diff --check`.
- The formal AI DAG is `A0 -> A1 -> (A2 || A3) -> A4 -> A5 -> A6`. A1 is the sole `ACTIVE` package; A2-A6 are `NOT STARTED`. This post-U6 chain is excluded from the closed USER_MODE `100/100` accounting.
...
The A0 evidence consists only of ADR-006 and its exact six-document documentation publication. It does not rerun the project gate; this commit must pass `git diff --check` and remain clean after its single commit. The fixed Tom design input is [`snowpeak008/Tom_doc@dd0f9ffc32d426735f7fb8960640e9b7ae9337bf`](https://github.com/snowpeak008/Tom_doc/tree/dd0f9ffc32d426735f7fb8960640e9b7ae9337bf), used only for non-sensitive import design.

`A1 — AI_PROVIDER_FOUNDATION` is the sole active package and owns only `src/VFXComposer.AI.Contracts/**`, `src/VFXComposer.AI.Providers/**`, `src/VFXComposer.AI.Tests/**`, `docs/schemas/desktop/vfxcomposer-ai-provider-config-v1.schema.json`, `VFXComposer.sln`, `eng/verify-phase2-schemas.py`, `eng/run-phase2-gate.ps1`, and `eng/phase2-baseline-roots.json`. The verifier is the only additional file beyond the three roots, schema, solution, runner, and baseline. All other paths are forbidden. A central package-management or external `.csproj` dependency is STOP, not a reason to widen ownership.

A1 evidence must prove canonical configuration; migration/future-version rejection; corrupt primary/backup fail-closed recovery; DPAPI no-plaintext and unreadable payload; URI policy; capability/channel mismatch; zero fallback; Tom `ApiKeyProtected` exclusion; Contracts/Providers/Tests and Broker/Worker/Unity/Desktop boundary; and redaction. It must prove a strict versioned atomic JSON/`.bak` store, DPAPI CurrentUser+SecretRef, configuration fingerprint, health/adapter-registry skeleton, safe Tom draft import, and `IAiGateway` without real Chat or Image HTTP. Logs, receipts, and exports contain no key/Auth/prompt/raw/base64; image output, if added later, is private cache only and never automatically written to Unity.

The first A1 writer is `STOPPED — ZERO WRITES`: the new AI schema would be desktop schema 23 while verifier line 57 hard-codes 22. A1 must change the expected total to 23, preserve `Phase 2=13`, `positive=14`, and `negative=236`, and add AI-schema structural validation with positive and negative cases; merely loosening the count is STOP.
